...
- **静态匹配**（默认）：只对映射表中精确存在的域名生效。学校数据库更新不频繁，绝大多数情况用这个就够了。
- **动态匹配**：除了精确匹配，还会自动识别同主域名下的子域名。比如映射表里有 `kns.cnki.net`，访问 `new.cnki.net` 也能自动跳转。而且动态匹配成功的新域名会**自动写入映射表**，切回静态模式后依然生效。

> 💡 **推荐用法**：平时用静态匹配。如果发现某个学术网站没被识别，临时切到动态匹配访问一下那个网站，新域名就自动记住了，然后切回静态即可。
...
A: 因为脚本需要在你访问**任意学术网站**时判断是否需要跳转。实际上，当域名不在映射表里时，脚本只执行几行判断就立刻退出了，完全不会影响页面加载和性能，甚至比页面上随便一个广告脚本都轻量得多 🪶
...
2. **临时切到动态匹配** — 打开设置，把匹配模式改为"动态匹配"，然后重新访问那个网站。动态模式会自动识别同主域名的子域名，并且匹配成功后会自动把新域名写入映射表。之后你可以切回静态匹配，这个域名已经被记住了 ✅
3. **手动添加映射** — 如果以上都不行，在设置中手动添加这个域名和对应的代理域名
