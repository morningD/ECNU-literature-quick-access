...
2. 想起来学校买了数据库，于是开始改 URL……
3. 把 `ieeexplore.ieee.org` 改成 `ieeexplore-ieee-org.proxy.ecnu.edu.cn`
4. 然后跳到 SSO 登录页，输入学号密码
5. 终于看到全文了，结果下一篇又要重复一遍 😭
...
Chrome 从 Tampermonkey 5.3+ 开始需要额外开启一个开关，否则用户脚本不会运行（你装了等于没装 😅）：

1. 打开 `chrome://extensions/`
2. 找到 Tampermonkey，点击 **详情**
3. 往下翻，找到 **允许访问文件网址**（Allow access to file URLs）打开
4. 最关键的：找到 **开发者模式** 旁边的 **允许用户脚本**（Allow User Scripts）开关，打开它！

> 📖 详情参考 [Tampermonkey 官方说明](https://www.tampermonkey.net/faq.php#Q209)

Edge 和 Firefox 用户不用操心这个 🎉
...
A: Chrome 用户请检查是否开启了 **允许用户脚本**（Allow User Scripts），详见上方 [Chrome 用户必看](#️-chrome-用户必看)。这是最常见的坑！
